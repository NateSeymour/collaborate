...
void complete_upgrade(SoupServerMessage *msg, gpointer _)
{
    ...
    SoupWebsocketConnection *conn = soup_websocket_connection_new(stream, uri, SOUP_WEBSOCKET_CONNECTION_SERVER, NULL, NULL, NULL);
...
void collaborate_room_manager_client_connection_handler(SoupServer *server, SoupServerMessage *msg, const char *path, GHashTable *query, gpointer user_data)
{
    ...
    res = soup_websocket_server_process_handshake(msg, NULL, NULL, NULL, NULL);
    if(!res)
    {
        g_error("[RoomManager] Handshake failed for client %s requesting stream access to %s.", remote_host, roomId);
    }

    g_signal_connect(msg, "wrote-informational", G_CALLBACK(complete_upgrade), NULL);
}
